...
class Requests extends Component {
    ...
    createRequest = (uniforms, description, cr) => {
        ...
        axios({
            method: 'post',
            url: 'https://gpstoque-api.herokuapp.com/request/',
            data: request
        }).then(() => {
            axios({
                method: 'get',
    ...
    render() {
        ...
        const products = this.props.requests.sort(sortKeys).map((product, index) => {
            if(!product.active) {
                return false
            }
        ...
        return (
            <RequestComponent>
                <FirstText>Você está na seção de solicitações.</FirstText>
                <ActionsWrapper>
                    <Button onClick={() => this.showCreate()} active={this.state.create}>Separação dos uniformes</Button>
                    <Button onClick={() => this.showList()} active={this.state.list}>Ver histórico de solicitações</Button>
                </ActionsWrapper>
                ...
                {this.state.create ? 
                    <ComponentWrapper>
                        <EditForm>
                            ...
                            <FieldsetDiv>
                                <label>Código</label>
                                <input type="number" value={this.props.requests.length + 1} disabled></input>
                            </FieldsetDiv>
                            <FieldsetDiv>
                ...
                {this.state.list && products.length > 0? 
                    <ComponentWrapper>
                        ...
                        {this.state.details ? 
                            <EditForm>
                                <SecondText>Você está vendo os detalhes da solicitação com o código "{this.state.productDetailsCode}".</SecondText>
                                <FieldsetDiv>
                                    <label>Código</label>
...
export default connect(mapStateToProps, { updateRequests, updateUniforms })(Requests);
